...
| 7 | `DIVERS-1` | Le moteur produit des programmes diversifiés selon la classe : pièces du socle inertes activées, mobilier manquant ajouté | `scripts/monitor-diversite.mjs` avant/après à graine égale ; chaque seuil d'émergence essayé puis figé ou écarté ; aucune régression de la suite | `in_progress` — repas fait le 25 septembre 2026 : coin adossé partout, salle autonome dès 90 m², solveur borné (`CLASSIFICATION_LOGEMENT.md` §7-9, `test-repas.mjs`) ; rejeu : révision du moteur + instantané dans l'archive rapide, forme en U **gelée** (26 septembre 2026) ; restent le studio, entrée, buanderie, cellier, local technique, garage, mobilier ; étalonnage humain gelé jusqu'avant release (`CLASSIFICATION_LOGEMENT.md` §3) |
| 8 | `PROGRAMME-BANDES` | Le programme d'un logement se déduit de sa surface par une table par bande ; le studio (12 m²) coexiste avec le programme actuel | Table validée puis canonique ; moniteur compare production et table par bande ; studio de 12 m² valide en mode lit ; suite verte, empreintes comparées | `in_progress` — étapes 1 à 4 faites le 26 septembre 2026 (table validée, donnée canonique, moniteur, planchers chambre / salle à manger / repas hébergé, `test-bandes-integration.mjs`) ; prochaine : le studio sous 25 m² en coexistence, mode lit (`TABLE_PROGRAMME_BANDES.md` §10-11) ; dette : cinq tests de mécanisme à réaligner avec la table |
